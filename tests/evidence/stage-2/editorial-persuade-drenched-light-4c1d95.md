...
## 品牌共性
- 风格：书卷。标题、正文与留白形成编辑节奏
- 表面语言：少阴影、细分隔；阅读正文重视行宽，操作区域重视扫描
- 形状：2px / 4px / 4px；同类组件全站一致。
...
当前预览：营销说服；其他页面不必采用同一用途。
色彩策略：用户明确选择“浸染”，各用途都保留此选择并适配区域。

### persuade / 营销说服
- 目的：理解价值、查看真实证明并决定是否行动
- 布局：从价值主张到具体证明再到行动；按需求选择主视觉和布局，不固定双栏 Hero。
- 密度：首屏主次明确；证明区可以密集，章节之间保留呼吸。
- 色彩：品牌区域可承担显著面积；主行动必须与所在背景区分。 当前适用策略：浸染。
...
- 色彩：默认克制；明确选择强色策略时，重点报告区用强色，输入与数据表面保持可读。 当前适用策略：浸染。
...
- 色彩：正文表面低干扰；强品牌色可用于封面或章节引导，链接与当前阅读位置可辨。 当前适用策略：浸染。
...
- 当前风格字形：衬线、横细竖粗、有书写感；正文用宋体保持阅读节奏，数据保留衬线数字
- 展示字体：`Georgia, "Source Serif 4", "Times New Roman", "Songti SC", "Noto Serif SC", STSong, SimSun, serif`
- 中文正文：`"Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif`
- 数据与代码：`Georgia, "Songti SC", ui-monospace, monospace`，配合 font-variant-numeric: tabular-nums。
- 排版角色：展示标题 / 页面标题 / 区块标题 / 正文 / 标签与控件 / 辅助说明 / 数据与代码；字号、行高、字距、行宽见下方字体与用途 token。
- 中文行宽：正文按全角字数控制（32em），不直接套用英文 65–75ch；阅读页面优先保证行长与段落节奏。
...
## 设计变量
### 强调色
- `--color-accent-base`: #6038af（亮） / #6a51ab（暗） — 主强调色（主按钮、激活链接）
- `--color-accent-hover`: #4b1698（亮） / #7e6bb9（暗） — 强调色悬停态
- `--color-accent-active`: #29045a（亮） / #b2a9da（暗） — 强调色按压态
- `--color-accent-subtle`: #e8e8ff（亮） / #100b27（暗） — 浅色强调背景（标签底色）
- `--color-accent-on-accent`: #FFFFFF — 主按钮上的对比前景文字（自适应 APCA）
- `--color-focus-ring`: #c0baff（亮） / #2f225b（暗） — 输入框/按钮聚焦环
- `--color-accent-2`: #8d1f80（亮） / #914085（暗） — 渐变搭档色（主色 ±40° 色相，与主色同明度色度）
- `--gradient-brand`: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)) — 品牌主渐变（Hero / 主 CTA 背景）
### 中性色
- `--color-bg-primary`: #faf9ff（亮） / #040209（暗） — 页面背景基底
- `--color-bg-secondary`: #ffffff（亮） / #09080f（暗） — 卡片 / 容器表面层
- `--color-bg-tertiary`: #f7f5ff（亮） / #16151a（暗） — 悬停态背景
- `--color-border`: #d8d4e9（亮） / #29282b（暗） — 边框与分割线
- `--color-text-primary`: #242328（亮） / #d1d0d6（暗） — 正文主要文字
- `--color-text-secondary`: #69686d（亮） / #abaaaf（暗） — 次要描述文字与小图标
- `--color-text-muted`: #8f8e95（亮） / #727077（暗） — 输入框占位字与禁用态文字
- `--color-text-emphasis`: #07070a（亮） / #eeedf3（暗） — 标题与高强调文字
- `--color-surface-raised`: #ffffff（亮） / #100f14（暗） — 浮层表面（弹窗 / Popover / 下拉）
### 交互态
- `--color-disabled-bg`: #faf9ff（亮） / #040209（暗） — 禁用态背景色
- `--color-disabled-text`: #8f8e95（亮） / #727077（暗） — 禁用态文字颜色（neutral-300 占位级，可辨识）
- `--color-disabled-border`: #d8d4e9（亮） / #29282b（暗） — 禁用态边框颜色
- `--focus-ring-shadow`: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent) — 聚焦环（颜色 + 宽度 + 偏移三件套）
### 辅助色
- `--color-surface-secondary`: #e6ddff（亮） / #140b28（暗） — 同源大面积辅助背景色
- `--color-surface-secondary-hover`: #c8b6ff（亮） / #251744（暗） — 同源辅助卡片/悬停层
- `--color-surface-secondary-active`: #a588f7（亮） / #3d286b（暗） — 同源辅助选中/高亮态
### 功能色
...
- `--color-info`: #0070cb（亮） / #2389d1（暗） — 提示信息与常规通知状态色
- `--color-info-subtle`: #cff3ff（亮） / #001326（暗） — 信息提示淡底背景
### 阴影与层级
...
- `--radius-sm`: 2px — 书卷品牌形状
- `--radius-md`: 4px — 书卷品牌形状
- `--radius-lg`: 4px — 书卷品牌形状
### 排版
...
### 页面用途
- `--type-display-size`: clamp(2.25rem, 4.8vw, 4rem) — 营销说服用途角色
- `--type-title-size`: 1.75rem — 营销说服用途角色
- `--type-section-size`: 1.375rem — 营销说服用途角色
- `--type-body-size`: 1rem — 营销说服用途角色
- `--type-label-size`: 0.875rem — 营销说服用途角色
- `--type-meta-size`: 0.75rem — 营销说服用途角色
- `--type-display-leading`: 1.16 — 营销说服用途角色
- `--type-body-leading`: 1.75 — 营销说服用途角色
- `--type-display-weight`: 700 — 营销说服用途角色
- `--type-display-tracking`: -0.025em — 营销说服用途角色
- `--measure-title`: 14em — 营销说服用途角色
- `--measure-prose`: 32em — 营销说服用途角色
- `--layout-content-width`: 72rem — 营销说服用途角色
- `--layout-section-gap`: 6rem — 营销说服用途角色
- `--layout-panel-padding`: 1.5rem — 营销说服用途角色
- `--layout-stack-gap`: 0.75rem — 营销说服用途角色
- `--layout-control-padding`: 0.5rem — 营销说服用途角色
- `--motion-feedback-duration`: 220ms — 营销说服用途角色
### 区域配色
- `--region-canvas-bg`: #ded7fe（亮） / #161127（暗） — 区域背景
- `--region-canvas-text`: #0a090e（亮） / #faf9ff（暗） — 区域正文
- `--region-canvas-text-secondary`: #302d3b（亮） / #c6c3d3（暗） — 区域次要文字
- `--region-canvas-border`: #ada9bf（亮） / #3a3847（暗） — 区域边框
- `--region-canvas-surface`: #eae3ff（亮） / #1e192e（暗） — 区域内层表面
- `--region-canvas-action`: #0b0b0e（亮） / #faf9fe（暗） — 区域内深色表面
- `--region-canvas-action-text`: #fafafc（亮） / #0a0a0b（暗） — 区域内行动文字
- `--region-canvas-focus`: #0b0b0e（亮） / #faf9fe（暗） — 区域内焦点环
- `--region-canvas-success`: #297d0f（亮） / #559247（暗） — 区域内成功状态色
- `--region-canvas-warning`: #c06300（亮） / #c87a14（暗） — 区域内警告状态色
- `--region-canvas-error`: #bb061e（亮） / #c64d49（暗） — 区域内错误状态色
- `--region-canvas-info`: #0070cb（亮） / #2389d1（暗） — 区域内信息状态色
- `--region-brand-bg`: #573899（亮） / #2f135e（暗） — 区域背景
- `--region-brand-text`: #faf8ff — 区域正文
- `--region-brand-text-secondary`: #c6c2d8 — 区域次要文字
- `--region-brand-border`: #78738d（亮） / #4e4961（暗） — 区域边框
- `--region-brand-surface`: #664ca7（亮） / #3d276c（暗） — 区域内层表面
- `--region-brand-action`: #faf8ff — 区域内浅色表面
- `--region-brand-action-text`: #0a0a0c — 区域内行动文字
- `--region-brand-focus`: #faf8ff — 区域内焦点环
- `--region-brand-success`: #65ac55（亮） / #559247（暗） — 区域内成功状态色
- `--region-brand-warning`: #e57f00（亮） / #c87a14（暗） — 区域内警告状态色
- `--region-brand-error`: #f76f68（亮） / #c64d49（暗） — 区域内错误状态色
- `--region-brand-info`: #2aa1f4（亮） / #2389d1（暗） — 区域内信息状态色
### 字体
- `--font-display`: Georgia, "Source Serif 4", "Times New Roman", "Songti SC", "Noto Serif SC", STSong, SimSun, serif — 书卷展示字体。衬线、横细竖粗、有书写感；正文用宋体保持阅读节奏，数据保留衬线数字
- `--font-body`: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif — 中文正文字体
- `--font-data`: Georgia, "Songti SC", ui-monospace, monospace — 数据与代码字体，配合 tabular-nums
- `--font-cn-display`: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif — 中文展示字体（缺字回退顺序）
- `--font-cn-body`: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif — 中文正文字体（缺字回退顺序）
- `--font-latin-display`: Georgia, "Source Serif 4", "Times New Roman", serif — 拉丁展示字体
### 间距阶梯
...
- `--space-card-padding`: 1.5rem — 营销说服内容表面内边距
- `--space-card-gap`: 0.75rem — 卡片间水平间距
- `--space-section-gap`: 6rem — 营销说服区块垂直间距
...
:root {
  /* 强调色 */
  --color-accent-base: #6038af; /* 主强调色（主按钮、激活链接） */
  --color-accent-hover: #4b1698; /* 强调色悬停态 */
  --color-accent-active: #29045a; /* 强调色按压态 */
  --color-accent-subtle: #e8e8ff; /* 浅色强调背景（标签底色） */
  --color-accent-on-accent: #FFFFFF; /* 主按钮上的对比前景文字（自适应 APCA） */
  --color-focus-ring: #c0baff; /* 输入框/按钮聚焦环 */
  --color-accent-2: #8d1f80; /* 渐变搭档色（主色 ±40° 色相，与主色同明度色度） */
  --gradient-brand: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)); /* 品牌主渐变（Hero / 主 CTA 背景） */
  /* 中性色 */
  --color-bg-primary: #faf9ff; /* 页面背景基底 */
  --color-bg-secondary: #ffffff; /* 卡片 / 容器表面层 */
  --color-bg-tertiary: #f7f5ff; /* 悬停态背景 */
  --color-border: #d8d4e9; /* 边框与分割线 */
  --color-text-primary: #242328; /* 正文主要文字 */
  --color-text-secondary: #69686d; /* 次要描述文字与小图标 */
  --color-text-muted: #8f8e95; /* 输入框占位字与禁用态文字 */
  --color-text-emphasis: #07070a; /* 标题与高强调文字 */
  --color-surface-raised: #ffffff; /* 浮层表面（弹窗 / Popover / 下拉） */
  /* 交互态 */
  --color-disabled-bg: #faf9ff; /* 禁用态背景色 */
  --color-disabled-text: #8f8e95; /* 禁用态文字颜色（neutral-300 占位级，可辨识） */
  --color-disabled-border: #d8d4e9; /* 禁用态边框颜色 */
  --focus-ring-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent); /* 聚焦环（颜色 + 宽度 + 偏移三件套） */
  /* 辅助色 */
  --color-surface-secondary: #e6ddff; /* 同源大面积辅助背景色 */
  --color-surface-secondary-hover: #c8b6ff; /* 同源辅助卡片/悬停层 */
  --color-surface-secondary-active: #a588f7; /* 同源辅助选中/高亮态 */
  /* 功能色 */
  ...
  --color-info: #0070cb; /* 提示信息与常规通知状态色 */
  --color-info-subtle: #cff3ff; /* 信息提示淡底背景 */
  /* 阴影与层级 */
  ...
  --motion-feedback-duration: 220ms; /* 营销说服用途角色 */
  /* 区域配色 */
  --region-canvas-bg: #ded7fe; /* 区域背景 */
  --region-canvas-text: #0a090e; /* 区域正文 */
  --region-canvas-text-secondary: #302d3b; /* 区域次要文字 */
  --region-canvas-border: #ada9bf; /* 区域边框 */
  --region-canvas-surface: #eae3ff; /* 区域内层表面 */
  --region-canvas-action: #0b0b0e; /* 区域内深色表面 */
  --region-canvas-action-text: #fafafc; /* 区域内行动文字 */
  --region-canvas-focus: #0b0b0e; /* 区域内焦点环 */
  --region-canvas-success: #297d0f; /* 区域内成功状态色 */
  --region-canvas-warning: #c06300; /* 区域内警告状态色 */
  --region-canvas-error: #bb061e; /* 区域内错误状态色 */
  --region-canvas-info: #0070cb; /* 区域内信息状态色 */
  --region-brand-bg: #573899; /* 区域背景 */
  --region-brand-text: #faf8ff; /* 区域正文 */
  --region-brand-text-secondary: #c6c2d8; /* 区域次要文字 */
  --region-brand-border: #78738d; /* 区域边框 */
  --region-brand-surface: #664ca7; /* 区域内层表面 */
  --region-brand-action: #faf8ff; /* 区域内浅色表面 */
  --region-brand-action-text: #0a0a0c; /* 区域内行动文字 */
  --region-brand-focus: #faf8ff; /* 区域内焦点环 */
  --region-brand-success: #65ac55; /* 区域内成功状态色 */
  --region-brand-warning: #e57f00; /* 区域内警告状态色 */
  --region-brand-error: #f76f68; /* 区域内错误状态色 */
  --region-brand-info: #2aa1f4; /* 区域内信息状态色 */
  /* 字体 */
  --font-display: Georgia, "Source Serif 4", "Times New Roman", "Songti SC", "Noto Serif SC", STSong, SimSun, serif; /* 书卷展示字体。衬线、横细竖粗、有书写感；正文用宋体保持阅读节奏，数据保留衬线数字 */
  --font-body: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif; /* 中文正文字体 */
  --font-data: Georgia, "Songti SC", ui-monospace, monospace; /* 数据与代码字体，配合 tabular-nums */
...
[data-theme="dark"] {
  --color-accent-base: #6a51ab;
  --color-accent-hover: #7e6bb9;
  --color-accent-active: #b2a9da;
  --color-accent-subtle: #100b27;
  --color-accent-on-accent: #FFFFFF;
  --color-focus-ring: #2f225b;
  --color-bg-primary: #040209;
  --color-bg-secondary: #09080f;
  --color-bg-tertiary: #16151a;
  --color-border: #29282b;
  --color-text-primary: #d1d0d6;
  --color-text-secondary: #abaaaf;
  --color-text-muted: #727077;
  --color-text-emphasis: #eeedf3;
  --color-disabled-bg: #040209;
  --color-disabled-text: #727077;
  --color-disabled-border: #29282b;
  --color-surface-secondary: #140b28;
  --color-surface-secondary-hover: #251744;
  --color-surface-secondary-active: #3d286b;
  ...
  --radius-md: 4px;
  --radius-lg: 4px;
  --font-display: Georgia, "Source Serif 4", "Times New Roman", "Songti SC", "Noto Serif SC", STSong, SimSun, serif;
  --font-body: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif;
  --font-data: Georgia, "Songti SC", ui-monospace, monospace;
  --font-cn-display: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif;
  --font-cn-body: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif;
  --font-latin-display: Georgia, "Source Serif 4", "Times New Roman", serif;
  --region-canvas-bg: #161127;
  --region-canvas-text: #faf9ff;
  --region-canvas-text-secondary: #c6c3d3;
  --region-canvas-border: #3a3847;
  --region-canvas-surface: #1e192e;
  --region-canvas-action: #faf9fe;
  --region-canvas-action-text: #0a0a0b;
  --region-canvas-focus: #faf9fe;
  --region-canvas-success: #559247;
  --region-canvas-warning: #c87a14;
  --region-canvas-error: #c64d49;
  --region-canvas-info: #2389d1;
  --region-brand-bg: #2f135e;
  --region-brand-text: #faf8ff;
  --region-brand-text-secondary: #c6c2d8;
  --region-brand-border: #4e4961;
  --region-brand-surface: #3d276c;
  --region-brand-action: #faf8ff;
  --region-brand-action-text: #0a0a0c;
  --region-brand-focus: #faf8ff;
  --region-brand-success: #559247;
  --region-brand-warning: #c87a14;
  --region-brand-error: #c64d49;
  --region-brand-info: #2389d1;
}
...
.ce-region-canvas {
  --color-bg-primary: var(--region-canvas-bg);
  --color-bg-secondary: var(--region-canvas-surface);
  --color-bg-tertiary: var(--region-canvas-surface);
  --color-surface-raised: var(--region-canvas-surface);
  --ce-card-bg: var(--region-canvas-surface);
  --color-text-primary: var(--region-canvas-text);
  --color-text-emphasis: var(--region-canvas-text);
  --color-text-secondary: var(--region-canvas-text-secondary);
  --color-text-muted: var(--region-canvas-text-secondary);
  --color-border: var(--region-canvas-border);
  --ce-border-c: var(--region-canvas-border);
  --color-success: var(--region-canvas-success);
  --color-warning: var(--region-canvas-warning);
  --color-error: var(--region-canvas-error);
  --color-info: var(--region-canvas-info);
  background: var(--region-canvas-bg);
  color: var(--region-canvas-text);
}
...
/* 书卷中文排版：衬线、横细竖粗、有书写感；正文用宋体保持阅读节奏，数据保留衬线数字
