...
## 设计变量
### 强调色
- `--color-accent-base`: #008930（亮） / #3a9552（暗） — 主强调色（主按钮、激活链接）
- `--color-accent-hover`: #006d14（亮） / #62ae77（暗） — 强调色悬停态
- `--color-accent-active`: #003e06（亮） / #aad7b5（暗） — 强调色按压态
- `--color-accent-subtle`: #ddf2dc（亮） / #061e07（暗） — 浅色强调背景（标签底色）
- `--color-accent-on-accent`: #FFFFFF — 主按钮上的对比前景文字（自适应 APCA）
- `--color-focus-ring`: #9dd5a0（亮） / #12491c（暗） — 输入框/按钮聚焦环
- `--color-accent-2`: #797500（亮） / #868500（暗） — 渐变搭档色（主色 ±40° 色相，与主色同明度色度）
- `--gradient-brand`: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)) — 品牌主渐变（Hero / 主 CTA 背景）
### 中性色
- `--color-bg-primary`: #f7fbf8（亮） / #020402（暗） — 页面背景基底
- `--color-bg-secondary`: #fefffe（亮） / #070a07（暗） — 卡片 / 容器表面层
- `--color-bg-tertiary`: #f2f9f3（亮） / #141714（暗） — 悬停态背景
- `--color-border`: #cfdbd0（亮） / #282928（暗） — 边框与分割线
- `--color-text-primary`: #212522（亮） / #cdd2ce（暗） — 正文主要文字
- `--color-text-secondary`: #666a66（亮） / #a7aca8（暗） — 次要描述文字与小图标
- `--color-text-muted`: #8b918c（亮） / #6e736e（暗） — 输入框占位字与禁用态文字
- `--color-text-emphasis`: #060806（亮） / #ebf0ec（暗） — 标题与高强调文字
- `--color-surface-raised`: #fefffe（亮） / #0d100e（暗） — 浮层表面（弹窗 / Popover / 下拉）
### 交互态
- `--color-disabled-bg`: #f7fbf8（亮） / #020402（暗） — 禁用态背景色
- `--color-disabled-text`: #8b918c（亮） / #6e736e（暗） — 禁用态文字颜色（neutral-300 占位级，可辨识）
- `--color-disabled-border`: #cfdbd0（亮） / #282928（暗） — 禁用态边框颜色
- `--focus-ring-shadow`: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent) — 聚焦环（颜色 + 宽度 + 偏移三件套）
### 辅助色
- `--color-surface-secondary`: #d6ebd9（亮） / #07150a（暗） — 同源大面积辅助背景色
- `--color-surface-secondary-hover`: #aacfb0（亮） / #122716（暗） — 同源辅助卡片/悬停层
- `--color-surface-secondary-active`: #78ae82（亮） / #204027（暗） — 同源辅助选中/高亮态
### 功能色
- `--color-success`: #607400（亮） / #788a26（暗） — 确认与成功反馈状态色
- `--color-success-subtle`: #eaf4ce（亮） / #101400（暗） — 成功反馈淡底背景
...
### 区域配色
- `--region-brand-bg`: #00812e（亮） / #004601（暗） — 区域背景
- `--region-brand-text`: #f0fff2 — 区域正文
- `--region-brand-text-secondary`: #b9cbbb — 区域次要文字
- `--region-brand-border`: #829985（亮） / #4b614f（暗） — 区域边框
- `--region-brand-surface`: #279549（亮） / #00581e（暗） — 区域内层表面
- `--region-brand-action`: #f1fef3 — 区域内浅色表面
- `--region-brand-action-text`: #090b09 — 区域内行动文字
- `--region-brand-focus`: #f1fef3 — 区域内焦点环
- `--region-brand-success`: #bfd474（亮） / #7c8e2c（暗） — 区域内成功状态色
- `--region-brand-warning`: #ffbe5c（亮） / #c87a14（暗） — 区域内警告状态色
- `--region-brand-error`: #530000（亮） / #d7635d（暗） — 区域内错误状态色
- `--region-brand-info`: #77d4ff（亮） / #2a8dd4（暗） — 区域内信息状态色
### 字体
...
:root {
  /* 强调色 */
  --color-accent-base: #008930; /* 主强调色（主按钮、激活链接） */
  --color-accent-hover: #006d14; /* 强调色悬停态 */
  --color-accent-active: #003e06; /* 强调色按压态 */
  --color-accent-subtle: #ddf2dc; /* 浅色强调背景（标签底色） */
  --color-accent-on-accent: #FFFFFF; /* 主按钮上的对比前景文字（自适应 APCA） */
  --color-focus-ring: #9dd5a0; /* 输入框/按钮聚焦环 */
  --color-accent-2: #797500; /* 渐变搭档色（主色 ±40° 色相，与主色同明度色度） */
  --gradient-brand: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)); /* 品牌主渐变（Hero / 主 CTA 背景） */
  /* 中性色 */
  --color-bg-primary: #f7fbf8; /* 页面背景基底 */
  --color-bg-secondary: #fefffe; /* 卡片 / 容器表面层 */
  --color-bg-tertiary: #f2f9f3; /* 悬停态背景 */
  --color-border: #cfdbd0; /* 边框与分割线 */
  --color-text-primary: #212522; /* 正文主要文字 */
  --color-text-secondary: #666a66; /* 次要描述文字与小图标 */
  --color-text-muted: #8b918c; /* 输入框占位字与禁用态文字 */
  --color-text-emphasis: #060806; /* 标题与高强调文字 */
  --color-surface-raised: #fefffe; /* 浮层表面（弹窗 / Popover / 下拉） */
  /* 交互态 */
  --color-disabled-bg: #f7fbf8; /* 禁用态背景色 */
  --color-disabled-text: #8b918c; /* 禁用态文字颜色（neutral-300 占位级，可辨识） */
  --color-disabled-border: #cfdbd0; /* 禁用态边框颜色 */
  --focus-ring-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent); /* 聚焦环（颜色 + 宽度 + 偏移三件套） */
  /* 辅助色 */
  --color-surface-secondary: #d6ebd9; /* 同源大面积辅助背景色 */
  --color-surface-secondary-hover: #aacfb0; /* 同源辅助卡片/悬停层 */
  --color-surface-secondary-active: #78ae82; /* 同源辅助选中/高亮态 */
  /* 功能色 */
  --color-success: #607400; /* 确认与成功反馈状态色 */
  --color-success-subtle: #eaf4ce; /* 成功反馈淡底背景 */
  ...
  --motion-feedback-duration: 220ms; /* 营销说服用途角色 */
  /* 区域配色 */
  --region-brand-bg: #00812e; /* 区域背景 */
  --region-brand-text: #f0fff2; /* 区域正文 */
  --region-brand-text-secondary: #b9cbbb; /* 区域次要文字 */
  --region-brand-border: #829985; /* 区域边框 */
  --region-brand-surface: #279549; /* 区域内层表面 */
  --region-brand-action: #f1fef3; /* 区域内浅色表面 */
  --region-brand-action-text: #090b09; /* 区域内行动文字 */
  --region-brand-focus: #f1fef3; /* 区域内焦点环 */
  --region-brand-success: #bfd474; /* 区域内成功状态色 */
  --region-brand-warning: #ffbe5c; /* 区域内警告状态色 */
  --region-brand-error: #530000; /* 区域内错误状态色 */
  --region-brand-info: #77d4ff; /* 区域内信息状态色 */
  /* 字体 */
  --font-display: -apple-system, "Segoe UI", "Inter", "PingFang SC", "Hiragino Sans GB", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 标准展示字体。中性无衬线、字面标准、笔画均匀；依靠字号与字重建立层级 */
  --font-body: "PingFang SC", "Hiragino Sans GB", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 中文正文字体 */
  --font-data: "SF Mono", Menlo, "Cascadia Code", Consolas, ui-monospace, monospace; /* 数据与代码字体，配合 tabular-nums */
...
[data-theme="dark"] {
  --color-accent-base: #3a9552;
  --color-accent-hover: #62ae77;
  --color-accent-active: #aad7b5;
  --color-accent-subtle: #061e07;
  --color-accent-on-accent: #FFFFFF;
  --color-focus-ring: #12491c;
  --color-bg-primary: #020402;
  --color-bg-secondary: #070a07;
  --color-bg-tertiary: #141714;
  --color-border: #282928;
  --color-text-primary: #cdd2ce;
  --color-text-secondary: #a7aca8;
  --color-text-muted: #6e736e;
  --color-text-emphasis: #ebf0ec;
  --color-disabled-bg: #020402;
  --color-disabled-text: #6e736e;
  --color-disabled-border: #282928;
  --color-surface-secondary: #07150a;
  --color-surface-secondary-hover: #122716;
  --color-surface-secondary-active: #204027;
  --color-success: #788a26;
  --color-success-subtle: #101400;
  ...
  --region-brand-bg: #004601;
  --region-brand-text: #f0fff2;
  --region-brand-text-secondary: #b9cbbb;
  --region-brand-border: #4b614f;
  --region-brand-surface: #00581e;
  --region-brand-action: #f1fef3;
  --region-brand-action-text: #090b09;
  --region-brand-focus: #f1fef3;
  --region-brand-success: #7c8e2c;
  --region-brand-warning: #c87a14;
  --region-brand-error: #d7635d;
  --region-brand-info: #2a8dd4;
}
