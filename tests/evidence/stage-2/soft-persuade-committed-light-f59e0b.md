...
## 品牌共性
- 风格：暖糖。饱满的字形、舒展的曲线和温和的分组
- 表面语言：柔和表面与圆角；操作密集时保留曲线语言并收紧间距
- 形状：14px / 20px / 20px；同类组件全站一致。
...
- 当前风格字形：人文无衬线、字面偏宽、字腔开、重心居中；中文圆体需可选展示字体，缺失时用人文无衬线
- 展示字体：`"SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif`
- 中文正文：`"Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif`
...
### 中性色
- `--color-bg-primary`: #fdf9f5（亮） / #050301（暗） — 页面背景基底
- `--color-bg-secondary`: #fffffe（亮） / #0c0906（暗） — 卡片 / 容器表面层
- `--color-bg-tertiary`: #fcf6ef（亮） / #181613（暗） — 悬停态背景
- `--color-border`: #e0d6ca（亮） / #2a2927（暗） — 边框与分割线
...
- `--color-surface-raised`: #fffffe（亮） / #110f0c（暗） — 浮层表面（弹窗 / Popover / 下拉）
### 交互态
- `--color-disabled-bg`: #fdf9f5（亮） / #050301（暗） — 禁用态背景色
- `--color-disabled-text`: #938e89（亮） / #75716b（暗） — 禁用态文字颜色（neutral-300 占位级，可辨识）
- `--color-disabled-border`: #e0d6ca（亮） / #2a2927（暗） — 禁用态边框颜色
- `--focus-ring-shadow`: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent) — 聚焦环（颜色 + 宽度 + 偏移三件套）
### 辅助色
- `--color-surface-secondary`: #f3e1ce（亮） / #1a0f04（暗） — 同源大面积辅助背景色
- `--color-surface-secondary-hover`: #ddbe9c（亮） / #2e1e09（暗） — 同源辅助卡片/悬停层
- `--color-surface-secondary-active`: #c09665（亮） / #4b3213（暗） — 同源辅助选中/高亮态
### 功能色
...
- `--radius-sm`: 14px — 暖糖品牌形状
- `--radius-md`: 20px — 暖糖品牌形状
- `--radius-lg`: 20px — 暖糖品牌形状
### 排版
...
### 区域配色
- `--region-brand-bg`: #9f5300（亮） / #5c2200（暗） — 区域背景
- `--region-brand-text`: #fff8eb — 区域正文
- `--region-brand-text-secondary`: #d3c3b1 — 区域次要文字
- `--region-brand-border`: #a28e78（亮） / #6a5742（暗） — 区域边框
- `--region-brand-surface`: #b26a00（亮） / #6e3800（暗） — 区域内层表面
- `--region-brand-action`: #fff8ec（亮） / #fff8ed（暗） — 区域内浅色表面
- `--region-brand-action-text`: #0c0a08 — 区域内行动文字
- `--region-brand-focus`: #fff8ec（亮） / #fff8ed（暗） — 区域内焦点环
- `--region-brand-success`: #90d181（亮） / #559247（暗） — 区域内成功状态色
- `--region-brand-warning`: #ffac80（亮） / #d56e48（暗） — 区域内警告状态色
- `--region-brand-error`: #420000（亮） / #d05a55（暗） — 区域内错误状态色
- `--region-brand-info`: #67c8ff（亮） / #2389d1（暗） — 区域内信息状态色
### 字体
- `--font-display`: "SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif — 暖糖展示字体。人文无衬线、字面偏宽、字腔开、重心居中；中文圆体需可选展示字体，缺失时用人文无衬线
- `--font-body`: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif — 中文正文字体
- `--font-data`: "SF Mono", Menlo, "Cascadia Code", Consolas, ui-monospace, monospace — 数据与代码字体，配合 tabular-nums
- `--font-cn-display`: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif — 中文展示字体（缺字回退顺序）
- `--font-cn-body`: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif — 中文正文字体（缺字回退顺序）
- `--font-latin-display`: "SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", sans-serif — 拉丁展示字体
### 间距阶梯
...
:root {
  ...
  --gradient-brand: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)); /* 品牌主渐变（Hero / 主 CTA 背景） */
  /* 中性色 */
  --color-bg-primary: #fdf9f5; /* 页面背景基底 */
  --color-bg-secondary: #fffffe; /* 卡片 / 容器表面层 */
  --color-bg-tertiary: #fcf6ef; /* 悬停态背景 */
  --color-border: #e0d6ca; /* 边框与分割线 */
  ...
  --color-surface-raised: #fffffe; /* 浮层表面（弹窗 / Popover / 下拉） */
  /* 交互态 */
  --color-disabled-bg: #fdf9f5; /* 禁用态背景色 */
  --color-disabled-text: #938e89; /* 禁用态文字颜色（neutral-300 占位级，可辨识） */
  --color-disabled-border: #e0d6ca; /* 禁用态边框颜色 */
  --focus-ring-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent); /* 聚焦环（颜色 + 宽度 + 偏移三件套） */
  /* 辅助色 */
  --color-surface-secondary: #f3e1ce; /* 同源大面积辅助背景色 */
  --color-surface-secondary-hover: #ddbe9c; /* 同源辅助卡片/悬停层 */
  --color-surface-secondary-active: #c09665; /* 同源辅助选中/高亮态 */
  /* 功能色 */
  ...
  --motion-feedback-duration: 220ms; /* 营销说服用途角色 */
  /* 区域配色 */
  --region-brand-bg: #9f5300; /* 区域背景 */
  --region-brand-text: #fff8eb; /* 区域正文 */
  --region-brand-text-secondary: #d3c3b1; /* 区域次要文字 */
  --region-brand-border: #a28e78; /* 区域边框 */
  --region-brand-surface: #b26a00; /* 区域内层表面 */
  --region-brand-action: #fff8ec; /* 区域内浅色表面 */
  --region-brand-action-text: #0c0a08; /* 区域内行动文字 */
  --region-brand-focus: #fff8ec; /* 区域内焦点环 */
  --region-brand-success: #90d181; /* 区域内成功状态色 */
  --region-brand-warning: #ffac80; /* 区域内警告状态色 */
  --region-brand-error: #420000; /* 区域内错误状态色 */
  --region-brand-info: #67c8ff; /* 区域内信息状态色 */
  /* 字体 */
  --font-display: "SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 暖糖展示字体。人文无衬线、字面偏宽、字腔开、重心居中；中文圆体需可选展示字体，缺失时用人文无衬线 */
  --font-body: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 中文正文字体 */
  --font-data: "SF Mono", Menlo, "Cascadia Code", Consolas, ui-monospace, monospace; /* 数据与代码字体，配合 tabular-nums */
...
[data-theme="dark"] {
  ...
  --color-bg-secondary: #0c0906;
  --color-bg-tertiary: #181613;
  --color-border: #2a2927;
  --color-text-primary: #d4d0cb;
  --color-text-secondary: #aeaaa5;
  --color-text-muted: #75716b;
  --color-text-emphasis: #f2eee9;
  --color-disabled-bg: #050301;
  --color-disabled-text: #75716b;
  --color-disabled-border: #2a2927;
  --color-surface-secondary: #1a0f04;
  --color-surface-secondary-hover: #2e1e09;
  --color-surface-secondary-active: #4b3213;
  ...
  --radius-md: 20px;
  --radius-lg: 20px;
  --font-display: "SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif;
  --font-body: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif;
  --font-data: "SF Mono", Menlo, "Cascadia Code", Consolas, ui-monospace, monospace;
  --font-cn-display: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif;
  --font-cn-body: "Hiragino Sans GB", "PingFang SC", "Noto Sans SC", "Microsoft YaHei", sans-serif;
  --font-latin-display: "SF Pro Rounded", "Nunito", "Segoe UI Variable", "Segoe UI", sans-serif;
  --region-brand-bg: #5c2200;
  --region-brand-text: #fff8eb;
  --region-brand-text-secondary: #d3c3b1;
  --region-brand-border: #6a5742;
  --region-brand-surface: #6e3800;
  --region-brand-action: #fff8ed;
  --region-brand-action-text: #0c0a08;
  --region-brand-focus: #fff8ed;
  --region-brand-success: #559247;
  --region-brand-warning: #d56e48;
  --region-brand-error: #d05a55;
...
/* 暖糖中文排版：人文无衬线、字面偏宽、字腔开、重心居中；中文圆体需可选展示字体，缺失时用人文无衬线
