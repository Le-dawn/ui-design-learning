...
- 色彩：默认克制；明确选择强色策略时，重点报告区用强色，输入与数据表面保持可读。 当前适用策略：全色板。
...
- 色彩：正文表面低干扰；强品牌色可用于封面或章节引导，链接与当前阅读位置可辨。 当前适用策略：全色板。
...
- `--color-bg-tertiary`: #f1f7ff（亮） / #14161a（暗） — 悬停态背景
...
### 辅助色
- `--color-surface-secondary`: #d0e5ff（亮） / #051029（暗） — 同源大面积辅助背景色
- `--color-surface-secondary-hover`: #a0c4ff（亮） / #0d1f45（暗） — 同源辅助卡片/悬停层
- `--color-surface-secondary-active`: #6b9cfb（亮） / #19346e（暗） — 同源辅助选中/高亮态
### 功能色
...
- `--region-auxiliary-bg`: #e1d7ff（亮） / #1b132e（暗） — 区域背景
- `--region-auxiliary-text`: #0a090f（亮） / #faf9ff（暗） — 区域正文
- `--region-auxiliary-text-secondary`: #302c3d（亮） / #c6c3d6（暗） — 区域次要文字
- `--region-auxiliary-border`: #afaac4（亮） / #3f3b4d（暗） — 区域边框
- `--region-auxiliary-surface`: #eee5ff（亮） / #241d37（暗） — 区域内层表面
- `--region-auxiliary-action`: #0b0a0f（亮） / #faf9ff（暗） — 区域内深色表面
- `--region-auxiliary-action-text`: #faf9fd（亮） / #0a0a0c（暗） — 区域内行动文字
- `--region-auxiliary-focus`: #0b0a0f（亮） / #faf9ff（暗） — 区域内焦点环
- `--region-auxiliary-success`: #297d0f（亮） / #559247（暗） — 区域内成功状态色
- `--region-auxiliary-warning`: #c06300（亮） / #c87a14（暗） — 区域内警告状态色
- `--region-auxiliary-error`: #bb061e（亮） / #c64d49（暗） — 区域内错误状态色
- `--region-auxiliary-info`: #0070cb（亮） / #2389d1（暗） — 区域内信息状态色
### 字体
...
:root {
  ...
  --focus-ring-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent); /* 聚焦环（颜色 + 宽度 + 偏移三件套） */
  /* 辅助色 */
  --color-surface-secondary: #d0e5ff; /* 同源大面积辅助背景色 */
  --color-surface-secondary-hover: #a0c4ff; /* 同源辅助卡片/悬停层 */
  --color-surface-secondary-active: #6b9cfb; /* 同源辅助选中/高亮态 */
  /* 功能色 */
  ...
  --region-auxiliary-bg: #e1d7ff; /* 区域背景 */
  --region-auxiliary-text: #0a090f; /* 区域正文 */
  --region-auxiliary-text-secondary: #302c3d; /* 区域次要文字 */
  --region-auxiliary-border: #afaac4; /* 区域边框 */
  --region-auxiliary-surface: #eee5ff; /* 区域内层表面 */
  --region-auxiliary-action: #0b0a0f; /* 区域内深色表面 */
  --region-auxiliary-action-text: #faf9fd; /* 区域内行动文字 */
  --region-auxiliary-focus: #0b0a0f; /* 区域内焦点环 */
  --region-auxiliary-success: #297d0f; /* 区域内成功状态色 */
  --region-auxiliary-warning: #c06300; /* 区域内警告状态色 */
  --region-auxiliary-error: #bb061e; /* 区域内错误状态色 */
  --region-auxiliary-info: #0070cb; /* 区域内信息状态色 */
  /* 字体 */
  --font-display: -apple-system, "Segoe UI", "Inter", "PingFang SC", "Hiragino Sans GB", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 标准展示字体。中性无衬线、字面标准、笔画均匀；依靠字号与字重建立层级 */
  --font-body: "PingFang SC", "Hiragino Sans GB", "Noto Sans SC", "Microsoft YaHei", sans-serif; /* 中文正文字体 */
  --font-data: "SF Mono", Menlo, "Cascadia Code", Consolas, ui-monospace, monospace; /* 数据与代码字体，配合 tabular-nums */
...
[data-theme="dark"] {
  ...
  --color-surface-secondary: #051029;
  --color-surface-secondary-hover: #0d1f45;
  --color-surface-secondary-active: #19346e;
  ...
  --region-auxiliary-bg: #1b132e;
  --region-auxiliary-text: #faf9ff;
  --region-auxiliary-text-secondary: #c6c3d6;
  --region-auxiliary-border: #3f3b4d;
  --region-auxiliary-surface: #241d37;
  --region-auxiliary-action: #faf9ff;
  --region-auxiliary-action-text: #0a0a0c;
  --region-auxiliary-focus: #faf9ff;
  --region-auxiliary-success: #559247;
  --region-auxiliary-warning: #c87a14;
  --region-auxiliary-error: #c64d49;
  --region-auxiliary-info: #2389d1;
}
...
.ce-region-auxiliary {
  --color-bg-primary: var(--region-auxiliary-bg);
  --color-bg-secondary: var(--region-auxiliary-surface);
  --color-bg-tertiary: var(--region-auxiliary-surface);
  --color-surface-raised: var(--region-auxiliary-surface);
  --ce-card-bg: var(--region-auxiliary-surface);
  --color-text-primary: var(--region-auxiliary-text);
  --color-text-emphasis: var(--region-auxiliary-text);
  --color-text-secondary: var(--region-auxiliary-text-secondary);
  --color-text-muted: var(--region-auxiliary-text-secondary);
  --color-border: var(--region-auxiliary-border);
  --ce-border-c: var(--region-auxiliary-border);
  --color-accent-base: var(--region-auxiliary-action);
  --color-accent-on-accent: var(--region-auxiliary-action-text);
  --color-accent-hover: color-mix(in srgb, var(--region-auxiliary-action) 86%, var(--region-auxiliary-text));
  --color-accent-active: color-mix(in srgb, var(--region-auxiliary-action) 72%, var(--region-auxiliary-text));
  --color-accent-subtle: color-mix(in srgb, var(--region-auxiliary-action) 16%, var(--region-auxiliary-bg));
  --color-focus-ring: var(--region-auxiliary-focus);
  --color-success: var(--region-auxiliary-success);
  --color-warning: var(--region-auxiliary-warning);
  --color-error: var(--region-auxiliary-error);
  --color-info: var(--region-auxiliary-info);
  background: var(--region-auxiliary-bg);
  color: var(--region-auxiliary-text);
}
