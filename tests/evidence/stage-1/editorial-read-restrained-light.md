...
当前预览：内容阅读；其他页面不必采用同一用途。
色彩策略：用户明确选择“克制”，各用途都保留此选择并适配区域。

### persuade / 营销说服
- 目的：理解价值、查看真实证明并决定是否行动
- 布局：从价值主张到具体证明再到行动；按需求选择主视觉和布局，不固定双栏 Hero。
- 密度：首屏主次明确；证明区可以密集，章节之间保留呼吸。
- 色彩：品牌区域可承担显著面积；主行动必须与所在背景区分。 当前适用策略：克制。
...
- 色彩：默认克制；明确选择强色策略时，重点报告区用强色，输入与数据表面保持可读。 当前适用策略：克制。
...
- 色彩：正文表面低干扰；强品牌色可用于封面或章节引导，链接与当前阅读位置可辨。 当前适用策略：克制。
...
- 色彩：色彩跟随作品和品牌区域；界面前景与媒体分离，确保导航可辨。 当前适用策略：克制。
...
- 当前策略：克制。中性画布；颜色集中在主操作与状态，不铺满区块。
...
- 中文行宽：正文按全角字数控制（34em），不直接套用英文 65–75ch；阅读页面优先保证行长与段落节奏。
...
## 设计变量
### 强调色
- `--color-accent-base`: #3368d9（亮） / #517dd5（暗） — 主强调色（主按钮、激活链接）
- `--color-accent-hover`: #1747bd（亮） / #749ae7（暗） — 强调色悬停态
- `--color-accent-active`: #072471（亮） / #b3cbfa（暗） — 强调色按压态
- `--color-accent-subtle`: #daedff（亮） / #061832（暗） — 浅色强调背景（标签底色）
- `--color-accent-on-accent`: #FFFFFF — 主按钮上的对比前景文字（自适应 APCA）
- `--color-focus-ring`: #9cc6ff（亮） / #1a3b71（暗） — 输入框/按钮聚焦环
- `--color-accent-2`: #874dc6（亮） / #9268c6（暗） — 渐变搭档色（主色 ±40° 色相，与主色同明度色度）
- `--gradient-brand`: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)) — 品牌主渐变（Hero / 主 CTA 背景）
### 中性色
- `--color-bg-primary`: #f9fafc（亮） / #030305（暗） — 页面背景基底
- `--color-bg-secondary`: #ffffff（亮） / #08090b（暗） — 卡片 / 容器表面层
- `--color-bg-tertiary`: #f5f7fb（亮） / #151617（暗） — 悬停态背景
- `--color-border`: #d4d8de（亮） / #28292a（暗） — 边框与分割线
...
- `--color-surface-raised`: #ffffff（亮） / #0f0f11（暗） — 浮层表面（弹窗 / Popover / 下拉）
### 交互态
- `--color-disabled-bg`: #f9fafc（亮） / #030305（暗） — 禁用态背景色
- `--color-disabled-text`: #8c8f95（亮） / #6e7278（暗） — 禁用态文字颜色（neutral-300 占位级，可辨识）
- `--color-disabled-border`: #d4d8de（亮） / #28292a（暗） — 禁用态边框颜色
- `--focus-ring-shadow`: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent) — 聚焦环（颜色 + 宽度 + 偏移三件套）
### 辅助色
- `--color-surface-secondary`: #dde5f4（亮） / #0d121a（暗） — 同源大面积辅助背景色
- `--color-surface-secondary-hover`: #b7c5de（亮） / #1b222f（暗） — 同源辅助卡片/悬停层
- `--color-surface-secondary-active`: #8c9fc1（亮） / #2d384c（暗） — 同源辅助选中/高亮态
### 功能色
...
### 页面用途
- `--type-display-size`: 2.25rem — 内容阅读用途角色
- `--type-title-size`: 1.75rem — 内容阅读用途角色
- `--type-section-size`: 1.375rem — 内容阅读用途角色
- `--type-body-size`: 1.0625rem — 内容阅读用途角色
- `--type-label-size`: 0.875rem — 内容阅读用途角色
- `--type-meta-size`: 0.8125rem — 内容阅读用途角色
- `--type-display-leading`: 1.4 — 内容阅读用途角色
- `--type-body-leading`: 1.9 — 内容阅读用途角色
- `--type-display-weight`: 650 — 内容阅读用途角色
- `--type-display-tracking`: 0em — 内容阅读用途角色
- `--measure-title`: 18em — 内容阅读用途角色
- `--measure-prose`: 34em — 内容阅读用途角色
- `--layout-content-width`: 68rem — 内容阅读用途角色
- `--layout-section-gap`: 4rem — 内容阅读用途角色
- `--layout-panel-padding`: 1.5rem — 内容阅读用途角色
- `--layout-stack-gap`: 1rem — 内容阅读用途角色
- `--layout-control-padding`: 0.5rem — 内容阅读用途角色
- `--motion-feedback-duration`: 160ms — 内容阅读用途角色
### 区域配色
- `--region-brand-bg`: #f0f5fd（亮） / #191c20（暗） — 区域背景
- `--region-brand-text`: #0a0a0b（亮） / #f9fafb（暗） — 区域正文
- `--region-brand-text-secondary`: #2d2f32（亮） / #c4c6c9（暗） — 区域次要文字
- `--region-brand-border`: #c0c3c8（亮） / #3d3e42（暗） — 区域边框
- `--region-brand-surface`: #f8fcff（亮） / #1f2125（暗） — 区域内层表面
- `--region-brand-action`: #3368d9 — 品牌强调色
- `--region-brand-action-text`: #fafafb（亮） / #fafafa（暗） — 区域内行动文字
- `--region-brand-focus`: #3368d9 — 区域内焦点环
### 字体
...
:root {
  /* 强调色 */
  --color-accent-base: #3368d9; /* 主强调色（主按钮、激活链接） */
  --color-accent-hover: #1747bd; /* 强调色悬停态 */
  --color-accent-active: #072471; /* 强调色按压态 */
  --color-accent-subtle: #daedff; /* 浅色强调背景（标签底色） */
  --color-accent-on-accent: #FFFFFF; /* 主按钮上的对比前景文字（自适应 APCA） */
  --color-focus-ring: #9cc6ff; /* 输入框/按钮聚焦环 */
  --color-accent-2: #874dc6; /* 渐变搭档色（主色 ±40° 色相，与主色同明度色度） */
  --gradient-brand: linear-gradient(135deg, var(--color-accent-base), var(--color-accent-2)); /* 品牌主渐变（Hero / 主 CTA 背景） */
  /* 中性色 */
  --color-bg-primary: #f9fafc; /* 页面背景基底 */
  --color-bg-secondary: #ffffff; /* 卡片 / 容器表面层 */
  --color-bg-tertiary: #f5f7fb; /* 悬停态背景 */
  --color-border: #d4d8de; /* 边框与分割线 */
  ...
  /* 交互态 */
  --color-disabled-bg: #f9fafc; /* 禁用态背景色 */
  --color-disabled-text: #8c8f95; /* 禁用态文字颜色（neutral-300 占位级，可辨识） */
  --color-disabled-border: #d4d8de; /* 禁用态边框颜色 */
  --focus-ring-shadow: 0 0 0 3px color-mix(in srgb, var(--color-focus-ring) 45%, transparent); /* 聚焦环（颜色 + 宽度 + 偏移三件套） */
  /* 辅助色 */
  --color-surface-secondary: #dde5f4; /* 同源大面积辅助背景色 */
  --color-surface-secondary-hover: #b7c5de; /* 同源辅助卡片/悬停层 */
  --color-surface-secondary-active: #8c9fc1; /* 同源辅助选中/高亮态 */
  /* 功能色 */
  ...
  --motion-feedback-duration: 160ms; /* 内容阅读用途角色 */
  /* 区域配色 */
  --region-brand-bg: #f0f5fd; /* 区域背景 */
  --region-brand-text: #0a0a0b; /* 区域正文 */
  --region-brand-text-secondary: #2d2f32; /* 区域次要文字 */
  --region-brand-border: #c0c3c8; /* 区域边框 */
  --region-brand-surface: #f8fcff; /* 区域内层表面 */
  --region-brand-action: #3368d9; /* 品牌强调色 */
  --region-brand-action-text: #fafafb; /* 区域内行动文字 */
  --region-brand-focus: #3368d9; /* 区域内焦点环 */
  /* 字体 */
  --font-display: Georgia, "Source Serif 4", "Times New Roman", "Songti SC", "Noto Serif SC", STSong, SimSun, serif; /* 书卷展示字体。衬线、横细竖粗、有书写感；正文用宋体保持阅读节奏，数据保留衬线数字 */
  --font-body: "Songti SC", "Noto Serif SC", STSong, SimSun, Georgia, serif; /* 中文正文字体 */
...
[data-theme="dark"] {
  --color-accent-base: #517dd5;
  --color-accent-hover: #749ae7;
  --color-accent-active: #b3cbfa;
  --color-accent-subtle: #061832;
  --color-accent-on-accent: #FFFFFF;
  --color-focus-ring: #1a3b71;
  --color-bg-primary: #030305;
  --color-bg-secondary: #08090b;
  --color-bg-tertiary: #151617;
  --color-border: #28292a;
  --color-text-primary: #ced1d6;
  --color-text-secondary: #a8abb0;
  --color-text-muted: #6e7278;
  --color-text-emphasis: #eceff4;
  --color-disabled-bg: #030305;
  --color-disabled-text: #6e7278;
  --color-disabled-border: #28292a;
  --color-surface-secondary: #0d121a;
  --color-surface-secondary-hover: #1b222f;
  --color-surface-secondary-active: #2d384c;
  ...
  --region-brand-bg: #191c20;
  --region-brand-text: #f9fafb;
  --region-brand-text-secondary: #c4c6c9;
  --region-brand-border: #3d3e42;
  --region-brand-surface: #1f2125;
  --region-brand-action: #3368d9;
  --region-brand-action-text: #fafafa;
  --region-brand-focus: #3368d9;
}
